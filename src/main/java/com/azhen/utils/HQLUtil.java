...
public class HQLUtil {
	...
	public static Long[] changeToLongArray(String ids) {
		...
		for (int i = 0; i < strIds.length; i++) {
			conditions[i] = Long.parseLong(strIds[i].trim());
		}
		return conditions;
	}
	...
	public static String createDeleteHQL(String entityName, String ids) {
		StringBuilder hqlStr = new StringBuilder("delete ").append(entityName).append(" as o where o.id in(");
		String[] strIds = ids.split(",");
		int i = 0;
	...
	public static String createDeleteOrHQL(String entityName, Set<String> attrNames) {
		StringBuilder hqlStr = new StringBuilder("delete ").append(entityName).append(" as o where ");
		Object[] attrNamesArray = attrNames.toArray();
		int i = 0;
		for (; i < attrNamesArray.length - 1; i++) {
	...
	public static String createUpdateHQL(String entityName, Set<String> propNames, Set<String> attrNames) {
		StringBuilder hqlStr = new StringBuilder("update ").append(entityName).append(" as o set ");
	...
	public static String createQueryHQL(String entityName, String ids) {
		StringBuilder hqlStr = new StringBuilder("from ").append(entityName).append(" as o where o.id in(");
		String[] strIds = ids.split(",");
		int i = 0;
		for (; i < strIds.length - 1; i++) {
			hqlStr.append("?,");
	...
	public static String createQueryHQL(String entityName, Set<String> attrNames) {
		StringBuilder hqlStr = new StringBuilder("from ").append(entityName).append(" as o where ");
		Object[] attrNamesArray = attrNames.toArray();
		int i = 0;
		for (; i < attrNamesArray.length - 1; i++) {
			hqlStr.append("o.").append(attrNamesArray[i]).append("=? and ");
	...
	public static String createLikeQueryHQL(String entityName, String likeAttr, Set<String>... attrNames) {
		StringBuilder hqlStr = new StringBuilder("from ").append(entityName).append(" as o where o.").append(likeAttr)
			.append(" like ?");
	...
	public static String createQueryTotalRecordHQL(String entityName, Set<String>... attrNames) {
		StringBuilder hqlStr = new StringBuilder("select count(o) from ").append(entityName).append(" as o ");
		if (attrNames.length > 0) {
			hqlStr.append("where ");
			Object[] attrNamesArray = attrNames[0].toArray();
			int i = 0;
	...
	public static String createQueryListHQL(String entityName, String orderAttr, String orderType, Set<String>... attrNames) {
		StringBuilder hqlStr = new StringBuilder("from ").append(entityName).append(" as o ");
		if (attrNames.length > 0) {
			hqlStr.append("where ");
			Object[] attrNamesArray = attrNames[0].toArray();
			int i = 0;
	 ...
	 * @return 查询条件
	 */
	public static StringBuilder createQueryWhereHQL(Map<String, Object> attr, Map<String, Object> likeAttrMap) {
	 ...
	 * @return
	 */
	public static StringBuilder createQueryTotalRecordHQL(String entityName, Map<String, Object> attr,
		Map<String, Object> likeAttrMap) {
		StringBuilder hqlStr = new StringBuilder("select count (*) from ").append(entityName).append(" as o ");
		if (attr != null) {
			hqlStr.append(" where 1=1 ");
			if (attr != null && attr.size() > 0) {
				for (String key : attr.keySet()) {
	...
	public static String createQueryNotInHQL(String entityName, String ids) {
		StringBuilder hqlStr = new StringBuilder("from ").append(entityName).append(" as o where 1=1 ");
